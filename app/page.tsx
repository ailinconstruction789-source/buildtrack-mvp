...
const AdminVisualizerView = dynamic(() => import('@/components/admin/AdminVisualizerView'));
const AdminHouseTypesView = dynamic(() => import('@/components/admin/AdminHouseTypesView'));
const AdminTasksView = dynamic(() => import('@/components/admin/AdminTasksView'));
const AdminProjectView = dynamic(() => import('@/components/admin/AdminProjectView'));
const AdminPlotView = dynamic(() => import('@/components/admin/AdminPlotView'));
const BillingQueueView = dynamic(() => import('@/components/BillingQueueView'));
const ProcurementDashboard = dynamic(() => import('@/components/ProcurementDashboard'));
...
export default function ConstructionApp() {
  ...
  const [markPlotModal, setMarkPlotModal] = useState<any>({ isOpen: false, plot: null, color: '', note: '' });
  ...
  // 🌟 State สำหรับ Popup Upload
  const [globalUploadState, setGlobalUploadState] = useState<{isUploading: boolean, isSuccess: boolean, message: string}>({ isUploading: false, isSuccess: false, message: '' });
  ...
  const [exclusionModal, setExclusionModal] = useState<any>({ isOpen: false, task: null, plots: [] });
  ...
  const handleUploadSlot = async (taskId: string, type: string, file: File | undefined | null) => {
    ...
    setGlobalUploadState({ isUploading: true, isSuccess: false, message: 'กำลังอัปโหลดรูปภาพ 2.5D... ห้ามปิดหน้าต่าง' });
    try {
    ...
    try {
      ...
      while (hasMore) {
        const { data: chunk, error } = await supabase
          .from('vw_billing_queue')
          .select('*')
          .order('latest_update_created_at', { ascending: false })
  ...
  useEffect(() => {
    if (view === 'billing-queue') {
      fetchBillingData();
    }
  }, [view, fetchBillingData]);
  ...
  const [batchPlots, setBatchPlots] = useState([{ id: '', house_type_id: '', foreman_name: '' }]);
  const [newUser, setNewUser] = useState({ name: '', role: 'Foreman' });
  const [newContractor, setNewContractor] = useState({ name: '', phone: '' });
  const [editingContractorId, setEditingContractorId] = useState<any>(null);
  const [editingContractor, setEditingContractor] = useState({ name: '', phone: '' });
  ...
  const handleZoomIn = useCallback(() => setMapZoom(prev => Math.min(prev + 0.2, 2.5)), []);
  const handleZoomOut = useCallback(() => setMapZoom(prev => Math.max(prev - 0.2, 0.5)), []);
  const handleZoomReset = useCallback(() => setMapZoom(1), []);
  ...
  const getPlotOverallStatus = useCallback((plotId: any) => {
    ...
    const actualAvg = Number(statusRecord.actual_avg) || 0;
    const plannedAvg = Number(statusRecord.planned_avg) || 0;
    const statusCode = statusRecord.status_code || 'none';

    if (statusCode === 'ready_for_sale') return { actual: actualAvg, planned: plannedAvg, status: 'ready_for_sale', label: 'พร้อมขาย/รอโอน', colors: 'bg-amber-100/90 border-amber-500 text-amber-800' };
    if (statusCode === 'completed') return { actual: actualAvg, planned: plannedAvg, status: 'completed', label: 'เสร็จสมบูรณ์', colors: 'bg-emerald-100/90 border-emerald-500 text-emerald-800' };
    if (statusCode === 'delayed') return { actual: actualAvg, planned: plannedAvg, status: 'delayed', label: 'ล่าช้ากว่าแผน', colors: 'bg-rose-100/90 border-rose-500 text-rose-800' };
    if (statusCode === 'ahead') return { actual: actualAvg, planned: plannedAvg, status: 'ahead', label: 'เร็วกว่าแผน', colors: 'bg-indigo-100/90 border-indigo-500 text-indigo-800' };
  ...
  const [copySourceHouseId, setCopySourceHouseId] = useState('');
  const [taskForm, setTaskForm] = useState({ id: '', task_name: '', task_order: '', cost: '' });
  const [isEditingTask, setIsEditingTask] = useState(false);

  const handleCopyTasks = async () => {
    if (!editingTaskHouseId || !copySourceHouseId) return showAlert('ข้อผิดพลาด', 'กรุณาเลือกแบบบ้านต้นฉบับ');
    if (editingTaskHouseId === copySourceHouseId) return showAlert('ข้อผิดพลาด', 'ไม่สามารถคัดลอกเข้าตัวเองได้');
    
    // Check if current house type already has tasks
    const existingTasks = taskTemplates.filter((t:any) => t.house_type_id === editingTaskHouseId);
    if (existingTasks.length > 0) {
      if (!confirm('แบบบ้านนี้มีงวดงานอยู่แล้ว การคัดลอกจะเพิ่มต่อท้ายงวดงานที่มีอยู่ ยืนยันดำเนินการต่อหรือไม่?')) return;
    }

    setIsSubmitting(true);
    try {
      const sourceTasks = taskTemplates.filter((t:any) => t.house_type_id === copySourceHouseId);
      if (sourceTasks.length === 0) {
        setIsSubmitting(false);
        return showAlert('ข้อผิดพลาด', 'แบบบ้านต้นฉบับยังไม่มีงวดงาน');
      }
      
      let nextOrder = existingTasks.length > 0 ? Math.max(...existingTasks.map((t:any) => t.task_order)) + 1 : 1;
      
      // We will sort source by task_order so they copy in sequence
      const sortedSource = [...sourceTasks].sort((a:any, b:any) => a.task_order - b.task_order);

      const newTasks = sortedSource.map(t => {
        const insertOrder = nextOrder++;
        return {
          house_type_id: editingTaskHouseId,
      ...
      const { error } = await supabase.from('task_templates').insert(newTasks);
      if (error) throw error;
      
      showToast('คัดลอกงวดงานสำเร็จ', 'success');
      await fetchAllData();
  ...
  // 🌟 Toggle Progress Counted
  const handleToggleProgressCounted = async (task: any) => {
    try {
      setIsSubmitting(true);
      const newValue = task.is_progress_counted === false ? true : false;
      const { error } = await supabase.from('task_templates').update({ is_progress_counted: newValue }).eq('id', task.id);
      if (error) throw error;
      await fetchAllData();
      showToast(newValue ? 'เปิดใช้นำมาคำนวณเรียบร้อย' : 'ปิดการนำมาคำนวณเรียบร้อย', 'success');
    } catch (e: any) {
      showAlert('ข้อผิดพลาด', e.message);
    } finally {
      setIsSubmitting(false);
  ...
  // 🌟 Exclusion Modal Logic (Bulk N/A) 🌟
  const handleOpenExclusionModal = async (task: any) => {
    try {
      setIsSubmitting(true);
      const { data: plotsData } = await supabase.from('plots').select('id, project_name').eq('house_type_id', task.house_type_id);
      
      const { data: ptaData } = await supabase.from('plot_task_assignments')
        .select('plot_id, is_excluded')
        .eq('task_template_id', task.id);
        
      const ptaMap: any = {};
      ptaData?.forEach(p => ptaMap[p.plot_id] = p.is_excluded);

      const formattedPlots = (plotsData || []).map(p => ({
        id: p.id,
        project_name: p.project_name,
        is_excluded: !!ptaMap[p.id]
      })).sort((a, b) => String(a.id).localeCompare(String(b.id), undefined, { numeric: true, sensitivity: 'base' }));

      setExclusionModal({ isOpen: true, task, plots: formattedPlots });
    } catch (e: any) {
      showAlert('ข้อผิดพลาด', 'ไม่สามารถโหลดข้อมูลแปลงได้');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleSaveExclusion = async () => {
    try {
      setIsSubmitting(true);
      const { task, plots } = exclusionModal;
      
      for (const p of plots) {
         const { data: existing } = await supabase.from('plot_task_assignments')
             .select('current_progress, actual_start_date, actual_end_date')
             .eq('plot_id', p.id).eq('task_template_id', task.id).maybeSingle();
             
         await supabase.from('plot_task_assignments').upsert({
             plot_id: p.id,
             task_template_id: task.id,
             is_excluded: p.is_excluded,
             current_progress: existing ? existing.current_progress : 0,
             actual_start_date: existing ? existing.actual_start_date : null,
             actual_end_date: existing ? existing.actual_end_date : null
         }, { onConflict: 'plot_id,task_template_id' });
      }
      
      await fetchAllData();
      showToast('บันทึกการตั้งค่าข้อยกเว้นเรียบร้อยแล้ว', 'success');
      setExclusionModal({ isOpen: false, task: null, plots: [] });
    } catch (e: any) {
      showAlert('ข้อผิดพลาด', e.message);
    } finally {
      setIsSubmitting(false);
  ...
  const handleLogout = () => {
    showConfirm('ยืนยันการออกจากระบบ', 'คุณแน่ใจหรือไม่ว่าต้องการออกจากระบบ?', async () => {
      ...
      setView('dashboard'); setSelectedProject(null);
      setProjects([]);
      setPlots([]);
  ...
  const handleExportCSV = () => {
    ...
    displayPlots.forEach(plot => { const statusInfo = getPlotOverallStatus(plot.id); csvContent += `${plot.project_name?.replace(/,/g, ' ')},${(plot.plot_name || plot.id)?.replace(/,/g, ' ')},${plot.type?.replace(/,/g, ' ')},${plot.foreman?.replace(/,/g, ' ') || 'ไม่ระบุ'},${plot.progress}%,${statusInfo.planned}%,${statusInfo.label}\n`; });
  ...
  const paintCell = useCallback((x: any, y: any) => setMapGrid((prev: any) => [...prev.filter((c: any) => !((c.type === 'plot' || c.type === 'road' || c.type === 'park') && c.x === x && c.y === y)), { id: `${x}-${y}`, type: mapTool, x, y, plotId: mapTool === 'plot' ? mapSelectedPlot : null }]), [mapTool, mapSelectedPlot]);
  const eraseCell = useCallback((x: any, y: any) => setMapGrid((prev: any) => prev.filter((c: any) => !((c.type === 'plot' || c.type === 'road' || c.type === 'park') && c.x === x && c.y === y))), []);
  ...
  const handleMouseDown = useCallback((x: any, y: any) => { if (!isEditMapMode) return; setIsDrawing(true); setLastDrawCell({ x, y }); if (mapTool === 'eraser') eraseCell(x, y); else if (mapTool === 'plot' || mapTool === 'road' || mapTool === 'park') { if (mapTool === 'plot' && !mapSelectedPlot) { setIsDrawing(false); return showAlert('แจ้งเตือน', 'เลือกรหัสแปลงก่อน'); } paintCell(x, y); } }, [isEditMapMode, mapTool, mapSelectedPlot, eraseCell, paintCell, showAlert]);
  const handleMouseEnter = useCallback((x: any, y: any) => { if (!isDrawing || !isEditMapMode) return; if (mapTool === 'fence' || mapTool === 'eraser') { if (lastDrawCell) { const dx = x - lastDrawCell.x, dy = y - lastDrawCell.y; if (Math.abs(dx) >= 1 && dy === 0) toggleFence('v', Math.max(x, lastDrawCell.x), y, mapTool === 'eraser' ? 'erase' : 'add'); else if (Math.abs(dy) >= 1 && dx === 0) toggleFence('h', x, Math.max(y, lastDrawCell.y), mapTool === 'eraser' ? 'erase' : 'add'); } if (mapTool === 'eraser') eraseCell(x, y); setLastDrawCell({ x, y }); } else { paintCell(x, y); setLastDrawCell({ x, y }); } }, [isDrawing, isEditMapMode, mapTool, lastDrawCell, toggleFence, eraseCell, paintCell]);
  const handleMouseUp = useCallback(() => { setIsDrawing(false); setLastDrawCell(null); }, []);
  const handleSaveMap = useCallback(async () => { setIsSubmitting(true); try { const finalGrid = [...mapGrid.filter(c => c.type !== 'config'), { id: 'GRID_CONFIG', type: 'config', cols: gridCols, rows: gridRows }]; await supabase.from('projects').update({ layout_data: finalGrid }).eq('name', selectedProject.name); showToast('บันทึกแผนผังเรียบร้อย!', "success"); await fetchAllData(); setSelectedProject((prev: any) => ({ ...prev, layout_data: finalGrid })); setIsEditMapMode(false); } catch (e: any) { showAlert('Error', (e as Error).message); } finally { setIsSubmitting(false); } }, [mapGrid, gridCols, gridRows, selectedProject, fetchAllData, showToast, showAlert]);
  ...
  const handleEditUsername = async (id: string, oldUsername: string) => {
    const newUsername = window.prompt(`เปลี่ยนชื่อ ID สำหรับ ${oldUsername}:`, oldUsername);
    if (!newUsername || newUsername.trim() === '' || newUsername.trim() === oldUsername) return;
    ...
    try {
      ...
      if (error) {
        if (error.message.includes('already exists')) throw new Error('ชื่อนี้มีคนใช้แล้วครับ');
        throw error;
      }
      
      const { data } = await supabase.from('users').select('*').order('role', { ascending: true }).order('username', { ascending: true });
      setAllUsers(data || []);
      showToast(`เปลี่ยนชื่อ ID จาก ${oldUsername} เป็น ${newUsername.trim()} สำเร็จ!`, "success");
    } catch (e: any) {
  ...
  const handleAddProject = async () => {
    ...
    try {
      ...
      setNewProjectName(''); await fetchAllData(); setView('dashboard'); setSelectedProject(null); showToast('สร้างโครงการใหม่เรียบร้อยแล้ว', "success");
    } catch (e: any) { showAlert('Error', (e as Error).message); } finally { setIsSubmitting(false); }
  };

  const handleDownloadTemplate = async () => {
    const XLSX = await import('xlsx');
    // 1. Sheet สำหรับเพิ่มข้อมูล (Main Sheet)
    const maxRows = 5;
    const mainData = [];
    for (let i = 0; i < maxRows; i++) {
      ...
      if (i === 0) {
        row['เลขที่แปลง'] = 'A01';
        row['แบบบ้าน'] = houseTypes?.[0]?.type_name || 'TYPE-A';
        row['โฟร์แมน'] = foremenList?.[0]?.username || '';
      } else if (i === 1) {
        row['เลขที่แปลง'] = 'A02';
        row['แบบบ้าน'] = houseTypes?.[1]?.type_name || 'TYPE-B';
        row['โฟร์แมน'] = foremenList?.[1]?.username || '';
      } else {
        row['เลขที่แปลง'] = '';
        row['แบบบ้าน'] = '';
        row['โฟร์แมน'] = '';
    ...
    // 2. Sheet สำหรับ Guide (อ้างอิงชื่อ)
    const guideMaxRows = Math.max(houseTypes?.length || 0, foremenList?.length || 0);
    const guideData = [];
    for (let i = 0; i < guideMaxRows; i++) {
      const row: any = {};
      row['📋 รายชื่อแบบบ้าน (คัดลอกไปวาง)'] = houseTypes?.[i]?.type_name || '';
      row['👷 รายชื่อโฟร์แมน (คัดลอกไปวาง)'] = foremenList?.[i]?.username || '';
      guideData.push(row);
    }

    const wsMain = XLSX.utils.json_to_sheet(mainData);
    wsMain['!cols'] = [{ wch: 15 }, { wch: 25 }, { wch: 25 }];

    const wsGuide = XLSX.utils.json_to_sheet(guideData);
    wsGuide['!cols'] = [{ wch: 40 }, { wch: 40 }];

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, wsMain, 'Add_Plots_Template');
    XLSX.utils.book_append_sheet(wb, wsGuide, 'Guide');
    
    XLSX.writeFile(wb, 'BuildTrack_AddPlot_Template.xlsx');
  };
  ...
  const handleImportExcel = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const wsname = wb.SheetNames[0]; // อ่าน Sheet แรกเสมอ
        const ws = wb.Sheets[wsname];
        const rows = XLSX.utils.sheet_to_json<any>(ws);

        if (rows && rows.length > 0) {
          const newBatchPlots = rows.map((row: any) => {
            const getVal = (keywords: string[]) => {
              const key = Object.keys(row).find(k => keywords.some(kw => k.toLowerCase().includes(kw)));
              return key ? String(row[key]).trim() : '';
            };

            const plotId = getVal(['plot', 'แปลง', 'เลข']);
            const typeStr = getVal(['house', 'type', 'แบบบ้าน']);
            const fmStr = getVal(['foreman', 'โฟร์แมน', 'ผู้ควบคุม']);

            const matchedType = houseTypes?.find(h => h.name?.toLowerCase() === typeStr.toLowerCase() || h.type_name?.toLowerCase() === typeStr.toLowerCase());
            const matchedFm = foremenList?.find(f => f.name?.toLowerCase() === fmStr.toLowerCase() || f.username?.toLowerCase() === fmStr.toLowerCase() || f.first_name?.toLowerCase() === fmStr.toLowerCase());

            return {
              id: plotId || '',
              house_type_id: matchedType?.id || '',
              foreman_name: matchedFm?.username || matchedFm?.name || fmStr || '',
              _invalidType: typeStr && !matchedType,
              _invalidFm: fmStr && !matchedFm
            };
          }).filter(p => p.id || p.house_type_id || p.foreman_name);

          // Validation logic
          const invalidRows = newBatchPlots.filter(p => p._invalidType || p._invalidFm);
          
          if (invalidRows.length > 0) {
             showAlert('นำเข้าข้อมูลไม่สำเร็จ', 'พบข้อมูลแบบบ้านหรือโฟร์แมนที่ไม่ถูกต้อง กรุณาอ้างอิงจาก Sheet Guide');
             return; // ปฏิเสธการนำเข้าทั้งหมดถ้าข้อมูลผิด
          }

          if (newBatchPlots.length > 0) {
            setBatchPlots(prev => {
              const cleanPrev = prev.filter(p => p.id || p.house_type_id || p.foreman_name);
              return [...cleanPrev, ...newBatchPlots];
            });
            showToast(`นำเข้าข้อมูล Excel จำนวน ${newBatchPlots.length} แปลง กรุณาตรวจสอบความถูกต้อง`, 'success');
          } else {
            showAlert('ไม่พบข้อมูล', 'ไม่พบข้อมูลในไฟล์ Excel หรือคอลัมน์ไม่ถูกต้อง');
          }
        } else {
          showAlert('ไม่พบข้อมูล', 'ไฟล์ Excel ว่างเปล่า');
        }
      } catch (err: any) {
        console.error(err);
        showAlert('ข้อผิดพลาด', 'ไม่สามารถอ่านไฟล์ Excel ได้ ลองเซฟเป็นไฟล์ .xlsx ใหม่อีกครั้ง');
      }
  ...
  const handleAddPlot = async () => {
    const invalidPlot = batchPlots.find(p => !p.id.trim() || !p.house_type_id);
    if (invalidPlot) return showAlert('แจ้งเตือน', 'กรอกรหัสแปลงและเลือกแบบบ้านให้ครบถ้วนทุกรายการ'); 
    
    const ids = batchPlots.map(p => p.id.trim().toUpperCase());
    if (new Set(ids).size !== ids.length) return showAlert('แจ้งเตือน', 'มีรหัสแปลงซ้ำกันในรายการที่กำลังเพิ่ม');

    setIsSubmitting(true);
    try {
      const plotsToInsert = batchPlots.map(p => ({
         id: `${selectedProject.name}-${p.id.trim().toUpperCase()}`,
         plot_name: p.id.trim().toUpperCase(),
         house_type_id: p.house_type_id,
         foreman_name: p.foreman_name,
         project_name: selectedProject.name
      }));
      
      const { error } = await supabase.from('plots').insert(plotsToInsert);
      if (error) throw error;
      
      setBatchPlots([{ id: '', house_type_id: '', foreman_name: '' }]); 
      await fetchAllData(); 
      setView('project-detail'); 
      showToast(`เพิ่มแปลงบ้าน ${batchPlots.length} แปลงเรียบร้อยแล้ว`, "success");
    } catch (e: any) { showAlert('Error', (e as Error).message); } finally { setIsSubmitting(false); }
  };
  ...
  const handleUpdateContractor = async () => {
    ...
    if (!editingContractor.name.trim() || !editingContractor.phone.trim()) return showAlert('แจ้งเตือน', 'กรุณากรอกข้อมูลช่างให้ครบ'); setIsSubmitting(true);
    try {
      await supabase.from('contractors').update({ name: editingContractor.name.trim(), phone: editingContractor.phone.trim() }).eq('id', editingContractorId);
      setEditingContractorId(null); setEditingContractor({ name: '', phone: '' }); await fetchAllData(); showToast('แก้ไขรายชื่อช่างเรียบร้อยแล้ว', "success");
    } catch (e: any) { showAlert('Error', (e as Error).message); } finally { setIsSubmitting(false); }
  };
  ...
  // 🌟 ฟังก์ชันลบแปลงบ้าน 🌟
  const handleLegacyProjectComplete = async () => {
    if (!selectedProject) return;
    const confirmName = window.prompt(`ยืนยันการตั้งค่าให้ทุกแปลงในโครงการ "${selectedProject.name}" เป็น "พร้อมขาย" และปิดงานก่อสร้างทั้งหมด 100% ใช่หรือไม่?\n\nคำเตือน: การกระทำนี้ไม่สามารถย้อนกลับได้ทีละหลายแปลง จะไม่มีการแจ้งเตือนและไม่แสดงใน Live Feed\n\nพิมพ์ชื่อโครงการ "${selectedProject.name}" เพื่อยืนยัน:`);
    if (confirmName !== selectedProject.name) {
      if (confirmName !== null) showAlert('แจ้งเตือน', 'ชื่อโครงการไม่ตรงกัน ยกเลิกการทำรายการ');
      return;
    }

    setIsSubmitting(true);
    try {
      const projPlots = plots.filter((p: any) => p.project_name === selectedProject.name);
      if (projPlots.length === 0) {
        setIsSubmitting(false);
        return showAlert('แจ้งเตือน', 'ไม่มีแปลงในโครงการนี้');
      }

      // 1. Mark plots ready for sale (original logic)
      ...
      const plotIds = projPlots.map((p: any) => p.id);
      await supabase.from('assignments').update({ status: 'completed' }).in('plot_id', plotIds);

      // 2. Call RPC to mark tasks as 100% silently (Process in chunks of 5 to avoid overloading)
      for (let i = 0; i < projPlots.length; i += 5) {
        const chunk = projPlots.slice(i, i + 5);
        await Promise.all(chunk.map(async (p: any) => {
          await supabase.rpc('bulk_mark_plot_completed', {
            p_plot_id: p.id,
            p_username: loggedInUser.username,
            p_role: loggedInUser.role
          });
        }));
      }

      showToast('อัปเดตข้อมูลโครงการเก่าสำเร็จ', 'success');
      await fetchAllData();
    } catch (e: any) {
      console.error(e);
      showAlert('ข้อผิดพลาด', e.message || 'เกิดข้อผิดพลาดในการทำรายการ');
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleLegacyProjectUndoTransfer = async () => {
    if (!selectedProject) return;
    if (!window.confirm(`ยืนยันการยกเลิกสถานะ "โอนแล้ว" ของทุกแปลงในโครงการ "${selectedProject.name}" ใช่หรือไม่?`)) return;
    ...
    try {
      ...
      showToast('เอาสถานะโอนออกสำเร็จ', 'success');
      await fetchAllData();
    } catch (e: any) {
      showAlert('ข้อผิดพลาด', e.message);
    } finally {
  ...
  const handleTogglePlotCompleted = async (plotId: any, currentStatus: boolean, actualProgress: number, hasCustomer: boolean) => {
    if (!currentStatus && !hasCustomer) {
      return showAlert('ไม่อนุญาต ❌', 'ไม่สามารถโอนบ้านได้เนื่องจากแปลงนี้ยังไม่มีลูกค้าจองครับ (สถานะลูกค้าเชื่อมโยงจากฝ่ายขาย)');
    }
  ...
  const handleForceCompleteAllTasks = async (plotId: any, houseTypeId: any) => {
    showConfirm('ยืนยันบังคับเสร็จสิ้น 100%', `คุณต้องการบังคับให้งานก่อสร้างทุกชิ้นในแปลง ${plotId} เสร็จสมบูรณ์ 100% ใช่หรือไม่? (สำหรับระบบบันทึกข้อมูลย้อนหลัง)`, async () => {
      setIsSubmitting(true);
      try {
        const tasksToComplete = taskTemplates.filter((t: any) => t.house_type_id === houseTypeId);
        if (tasksToComplete.length === 0) throw new Error('ไม่พบเทมเพลตงานสำหรับแบบบ้านนี้');

        const newUpdates = tasksToComplete.map((task: any) => ({
          plot_id: plotId,
          task_template_id: task.id,
          progress: 100,
          user_name: loggedInUser?.username,
          role: currentUserRole,
          action: 'บังคับเสร็จสิ้น 100% (Admin)'
        }));

        const { error: updateError } = await supabase.from('task_updates').insert(newUpdates);
        if (updateError) throw updateError;

        const { error: plotError } = await supabase.from('plots').update({ is_completed: true, progress: 100 }).eq('id', plotId);
        if (plotError) throw plotError;

        await fetchAllData();
        closeDialog();
        showToast(`แปลง ${plotId} ถูกบังคับให้เสร็จสิ้น 100% เรียบร้อยแล้ว`, "success");
      } catch (e: any) {
        showAlert('ข้อผิดพลาด', e.message);
      }
      setIsSubmitting(false);
  ...
  const handleUpdatePlot = async () => {
    ...
    try {
      ...
      if (selectedProject) {
        const { data: updatedProj } = await supabase.from('projects').select('*').eq('name', selectedProject.name).maybeSingle();
  ...
  const handleMarkPlot = (plot: any) => {
    setMarkPlotModal({ isOpen: true, plot, color: plot.highlight_color || '', note: plot.highlight_note || '' });
  };

  const handleSavePlotMarking = async () => {
    try {
      setIsSubmitting(true);
      const { error } = await supabase
        .from('plots')
        .update({ 
          highlight_color: markPlotModal.color || null,
          highlight_note: markPlotModal.note || null
        })
        .eq('id', markPlotModal.plot.id);
      
      if (error) throw error;
      
      setMarkPlotModal({ ...markPlotModal, isOpen: false });
      await fetchAllData();
      showToast('บันทึกการมาร์กแปลงเรียบร้อยแล้ว');
    } catch (e: any) {
      showAlert('ข้อผิดพลาด', e.message);
    } finally {
      setIsSubmitting(false);
  ...
  const handleToggleProjectStatus = async (projName: string, currentStatus: boolean) => {
    const newStatus = !currentStatus;
    const actionText = newStatus ? 'ปิดโครงการ' : 'เปิดโครงการ';
    showConfirm(
      `ยืนยัน${actionText}?`,
      `คุณต้องการ${actionText} "${projName}" ใช่หรือไม่?\n\nโครงการที่ถูกปิดจะไม่แสดงในหน้าก่อสร้าง แต่ข้อมูลยังอยู่ครบในฝ่ายขาย`,
      async () => {
        setIsSubmitting(true);
        try {
          const { error } = await supabase
          ...
          await fetchAllData();
          closeDialog();
          showToast(`${actionText} ${projName} เรียบร้อยแล้ว`, "success");
        } catch (e: any) {
          showAlert('ข้อผิดพลาด', e.message);
        } finally {
          setIsSubmitting(false);
  ...
  const handleUploadOverviewImage = async (file: File) => {
    ...
    setGlobalUploadState({ isUploading: true, isSuccess: false, message: 'กำลังบีบอัดและอัปโหลดรูปรวมหน้างาน... ห้ามปิดหน้าต่าง' });
    try {
      const comp = await compressImageNative(file);
      const safeId = String(selectedPlot.id).replace(/[^a-zA-Z0-9-]/g, '') || 'plot_images';
      const path = `${safeId}/overview-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      const { error } = await supabase.storage.from('task_images').upload(path, comp);
      ...
      await fetchAllData();
      setGlobalUploadState({ isUploading: false, isSuccess: true, message: 'อัปโหลดรูปรวมหน้างานเสร็จสิ้น!' });
      setTimeout(() => setGlobalUploadState({ isUploading: false, isSuccess: false, message: '' }), 2000);
    } catch (e: any) {
      setGlobalUploadState({ isUploading: false, isSuccess: false, message: '' });
      showAlert('Error', (e as Error).message);
    }
  ...
  const handleAdminUndoLatest = async (taskTemplateId: string, plotId: string) => {
    showConfirm(
      'แอดมิน: ยืนยันการย้อนสถานะล่าสุด ⚠️',
      'คุณแน่ใจหรือไม่ว่าต้องการ "ลบประวัติการอัปเดตล่าสุด" ของงานนี้? ระบบจะย้อนสถานะกลับไปยังประวัติก่อนหน้า (Undo)',
      async () => {
        setIsSending(true);
        try {
          // 1. Fetch current updates sorted by latest first
          const { data: currentUpdates } = await supabase.from('task_updates')
            .select('*')
            .eq('task_template_id', taskTemplateId)
            .eq('plot_id', plotId)
          ...
          // 2. Delete the latest update record
          const { error: deleteError } = await supabase.from('task_updates')
            .delete()
          ...
          setProgressValue(previousUpdate ? previousUpdate.progress : 0);
  ...
  const handleAdminResetToZero = async (taskTemplateId: string, plotId: string) => {
    showConfirm(
      ...
      async () => {
        setIsSending(true);
        try {
          // 1. Insert history first so DB trigger fires BEFORE our final state override
          await supabase.from('task_updates').insert([{ 
            plot_id: plotId, 
            task_template_id: taskTemplateId, 
            user_name: loggedInUser.username, 
            role: 'Admin', 
            action: 'แอดมินย้อนสถานะงาน (Reset)', 
          ...
          showAlert('สำเร็จ ✨', 'ย้อนสถานะงานกลับไปเริ่มต้นใหม่ที่ 0% เรียบร้อยแล้วครับ');
        } catch (e: any) {
          showAlert('Error', (e as Error).message);
        }
        setIsSending(false);
  ...
  const handleOpenBillingExport = async (taskItem: any) => {
    setSelectedTask(taskItem);
    const plot = projects.find((p: any) => p.name === taskItem.project_name)?.plots?.find((p: any) => p.id === taskItem.plot_id);
    setSelectedPlot(plot);
    
    // Fetch all updates (with images) for this specific plot and task directly from the database
    const { data: taskUpdates } = await supabase.from('task_updates')
      .select('*')
      .eq('plot_id', taskItem.plot_id)
      .eq('task_template_id', taskItem.task_template_id)
      .order('created_at', { ascending: true });
    ...
    if (taskUpdates) {
      ...
      setAllTaskImages(imgs); 
      setSelectedExportImages(imgs); 
    }
  ...
  const isSales = currentUserRole?.toLowerCase() === 'sales';
  ...
  const inspectionQueue = useMemo(() => {
      ...
      const queue = (inspectionQueueView || []).filter((upd: any) => {
        // Ensure plot and task exist (filter out orphans)
        const plot = plots.find(p => String(p.id) === String(upd.plot_id));
        const task = taskTemplates.find(t => String(t.id) === String(upd.task_template_id));
        if (!plot || !task) return false;
        // Only show items for current project
        if (selectedProject?.name && selectedProject.name !== 'all' && upd.project_name !== selectedProject.name) return false;
        
        const isPendingSEItem = isSiteEngineer && upd.statusFor === 'Site Engineer';
        const isPendingQCItem = isQC && upd.statusFor === 'QC';
        const isAdminView = (isAdmin || isOwner) && (upd.statusFor === 'Site Engineer' || upd.statusFor === 'QC');
        const isRejectedAdminView = (isAdmin || isOwner) && upd.isRejected;
        const isRejectedForemanItem = isForeman && upd.isRejected && upd.foreman === loggedInUser?.username;
        
        return isPendingSEItem || isPendingQCItem || isAdminView || isRejectedAdminView || isRejectedForemanItem;
      }).map((upd: any) => ({
        ...upd,
      ...
      queue.sort((a: any, b: any) => { 
        if (inspectionSort === 'plot') return (a.plot_id || '').localeCompare(b.plot_id || ''); 
        return b.time - a.time; 
      });
      return queue;
    }, [inspectionQueueView, isSiteEngineer, isQC, isAdmin, isOwner, isForeman, loggedInUser?.username, inspectionSort, selectedProject?.name]);
  ...
  if (view === 'house-detail' && selectedPlot) {
    ...
    plotTasks.forEach(task => {
      if (task.is_progress_counted === false) return; // ข้ามการคำนวณวันสำหรับงานที่ไม่นำมาคิด
  ...
  // 🌟 Derived state for Presentation Modal
  const presentationPlots = plots.filter((p: any) => p.project_name === selectedProject?.name);
  ...
  return (
    <div className={`min-h-screen font-sans text-[#1d1d1f] transition-all duration-500 ${isMobilePreview ? 'bg-slate-900 flex items-center justify-center py-4 sm:py-10' : 'bg-[#f5f5f7]'}`}>
      ...
      <div className={`${isMobilePreview ? '@container w-[390px] h-[844px] bg-slate-50 border-[14px] border-slate-950 rounded-[3rem] shadow-2xl relative overflow-hidden flex flex-col' : '@container flex h-screen w-full overflow-hidden'} print:hidden`}>
        ...
        {markPlotModal.isOpen && (
          <div className="absolute inset-0 z-[600] bg-black/40 backdrop-blur-xl flex items-center justify-center p-4 fixed transition-all">
            <div className="bg-white rounded-[2rem] shadow-2xl max-w-sm w-full p-6 space-y-5 animate-in zoom-in-95 duration-200">
              <div><h3 className="text-xl font-black text-slate-800 italic uppercase">มาร์กแปลง (Highlight Plot)</h3><p className="text-sm text-slate-500 font-bold tracking-widest">{markPlotModal.plot?.plot_name || markPlotModal.plot?.id}</p></div>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-black text-slate-500 mb-2 uppercase tracking-widest">เลือกสี (Color)</label>
                  <div className="flex flex-wrap gap-2">
                    {['red', 'orange', 'yellow', 'green', 'blue', 'purple', 'pink'].map(color => (
                      <button 
                        key={color} 
                        onClick={() => setMarkPlotModal({ ...markPlotModal, color })}
                        className={`w-10 h-10 rounded-full border-2 shadow-sm transition-all flex items-center justify-center
                          ...
                          ${markPlotModal.color === color ? 'ring-4 ring-slate-800 scale-110' : 'hover:scale-105'}
                        `}
                      >
                        {markPlotModal.color === color && <Check size={16} className="text-white" />}
                      </button>
                    ))}
                    <button 
                      onClick={() => setMarkPlotModal({ ...markPlotModal, color: '' })}
                      className={`px-3 py-1 text-[10px] font-bold rounded-full border-2 transition-all flex items-center justify-center
                        ${markPlotModal.color === '' ? 'bg-slate-800 text-white border-slate-900 ring-4 ring-slate-800/30' : 'bg-slate-100 text-slate-500 border-slate-200 hover:bg-slate-200'}
                      `}
                    >
                      ไม่มีสี
                    </button>
                ...
                <div>
                  <label className="block text-sm font-black text-slate-500 mb-1 uppercase tracking-widest">หมายเหตุ (Note)</label>
                  <input type="text" value={markPlotModal.note} onChange={(e) => setMarkPlotModal({ ...markPlotModal, note: e.target.value })} placeholder="เหตุผลที่มาร์ก..." className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold text-slate-700 outline-none focus:border-indigo-500" />
                </div>
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setMarkPlotModal({ ...markPlotModal, isOpen: false })} className="flex-1 px-4 py-3 bg-slate-100 text-slate-500 font-bold rounded-xl hover:bg-slate-200 transition-colors">ยกเลิก</button>
                <button onClick={handleSavePlotMarking} disabled={isSubmitting} className="flex-1 px-4 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-[0_0_15px_rgba(79,70,229,0.4)] transition-all disabled:opacity-50 flex items-center justify-center">
                  {isSubmitting ? <Loader2 size={18} className="animate-spin" /> : 'บันทึกมาร์ก'}
                </button>
              </div>
            </div>
          </div>
        ...
        {dialogConfig.isOpen && (
          <div className="fixed inset-0 z-[600] bg-black/40 backdrop-blur-xl flex items-center justify-center p-4 transition-all"><div className="bg-white rounded-[2rem] shadow-2xl min-w-[320px] max-w-sm w-full p-6 text-center space-y-4"><div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-2 ${dialogConfig.type === 'confirm' ? 'bg-rose-100 text-rose-600' : 'bg-blue-100 text-blue-600'}`}><AlertTriangle size={32} /></div><h3 className="text-xl font-black">{dialogConfig.title}</h3><p className="text-slate-500 font-medium">{dialogConfig.message}</p><div className="flex gap-3 w-full mt-4">{dialogConfig.type === 'confirm' ? (<><button onClick={closeDialog} className="flex-1 bg-slate-100 text-slate-600 font-bold py-3.5 rounded-xl hover:bg-slate-200">ยกเลิก</button><button onClick={dialogConfig.onConfirm || undefined} disabled={isSubmitting} className="flex-1 bg-rose-600 text-white font-bold py-3.5 rounded-xl hover:bg-rose-700 disabled:opacity-70 flex items-center justify-center gap-2">{isSubmitting ? <Loader2 size={18} className="animate-spin" /> : 'ยืนยัน'}</button></>) : (<button onClick={closeDialog} className="w-full bg-slate-800 text-white font-bold py-3.5 rounded-xl">รับทราบ</button>)}</div></div></div>
        )}
        ...
        <div className="flex-1 flex flex-col h-full bg-slate-50 relative overflow-hidden">
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-2">
                  {view === 'project-detail' || view === 'house-detail' || view === 'task-progress' || view === 'defect-progress' ? (
                    ...
                    <div className="flex items-center gap-2 cursor-pointer" onClick={() => { setView('dashboard'); setSelectedProject(null); }}>
          ...
          {!isMobileLayout && (
            <header className="bg-white border-b border-slate-200 p-3 sm:p-4 shrink-0 shadow-sm z-[100] flex justify-between items-center gap-4 relative">
              ...
              <div className="flex items-center">
                 {view === 'project-detail' && (
                    <button onClick={() => { setView('dashboard'); setSelectedProject(null); }} className="text-sm font-bold text-slate-500 hover:text-blue-600 flex items-center gap-1.5 hover:-translate-x-1 transition-transform">
            ...
            <div className="w-full">
              ...
              {view === 'house-detail' && selectedPlot && (
                <HouseDetailView
                  ...
                  handleTogglePlotCompleted={handleTogglePlotCompleted}
                  handleForceCompleteAllTasks={handleForceCompleteAllTasks}
              ...
              {view === 'billing-queue' && (isAdmin || isProcurement || isOwner) && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <BillingQueueView
                    ...
                    onExportClick={handleOpenBillingExport}
                  />
                </div>
              )}

              {/* 🏢 View: Procurement Dashboard */}
              {view === 'procurement-dashboard' && (isAdmin || isProcurement || isOwner) && (
                        ...
                        <div key={c.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                          {editingContractorId === c.id ? (
                            <div className="flex flex-col sm:flex-row gap-2 w-full">
                              <input type="text" value={editingContractor.name} onChange={(e) => setEditingContractor({ ...editingContractor, name: e.target.value })} placeholder="ชื่อช่าง..." className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold outline-none focus:border-emerald-500" />
                              <input type="text" value={editingContractor.phone} onChange={(e) => setEditingContractor({ ...editingContractor, phone: e.target.value })} placeholder="เบอร์โทรศัพท์..." className="flex-[0.7] bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold outline-none focus:border-emerald-500" />
                              <div className="flex gap-2 justify-end mt-2 sm:mt-0">
                                <button onClick={handleUpdateContractor} disabled={isSubmitting} className="bg-emerald-600 text-white p-2 rounded-lg hover:bg-emerald-700 shadow-sm"><Check size={16} /></button>
                                <button onClick={() => setEditingContractorId(null)} className="bg-slate-200 text-slate-600 p-2 rounded-lg hover:bg-slate-300 shadow-sm"><X size={16} /></button>
                              </div>
                            </div>
                          ) : (
                            <>
                              ...
                              <div className="flex gap-2">
                                <button onClick={() => { setEditingContractorId(c.id); setEditingContractor({ name: c.name, phone: c.phone || '' }); }} className="text-slate-400 hover:text-blue-500 p-2 bg-white rounded-lg shadow-sm"><Edit2 size={16} /></button>
              ...
              {view === 'store-dashboard' && (isStore || isAdmin) && (
                <div className="animate-in slide-in-from-bottom duration-300 mx-auto mt-4 sm:mt-8 px-4 sm:px-0">
                  ...
                  <MaterialStoreDashboard 
                    ...
                    setMaterialReceipts={setMaterialReceipts}
                  />
                </div>
              )}
              ...
              {view === 'admin-visualizer' && isAdmin && (
                <AdminVisualizerView
                  ...
                  taskTemplates={taskTemplates}
                  editingHouseType={editingHouseType}
                  setEditingHouseType={setEditingHouseType}
                  visualConfig={visualConfig}
                  setVisualConfig={setVisualConfig}
                  simulatedStatus={simulatedStatus}
                  setSimulatedStatus={setSimulatedStatus}
                  isSubmitting={isSubmitting}
                  isUploadingLayer={isUploadingLayer}
                  handleSave25DConfig={handleSave25DConfig}
              ...
              {view === 'admin-house-types' && isAdmin && (
                <AdminHouseTypesView
                  ...
                  setHouseTypeForm={setHouseTypeForm}
                  houseTypes={houseTypes}
                  isSubmitting={isSubmitting}
                  handleSaveHouseType={handleSaveHouseType}
                />
              )}
              {view === 'admin-tasks' && isAdmin && (
                <AdminTasksView
                  setView={setView}
                  setSelectedProject={setSelectedProject}
                  editingTaskHouseId={editingTaskHouseId}
                  setEditingTaskHouseId={setEditingTaskHouseId}
                  houseTypes={houseTypes}
                  taskTemplates={taskTemplates}
                  taskForm={taskForm}
                  ...
                  handleToggleProgressCounted={handleToggleProgressCounted}
                  handleOpenExclusionModal={handleOpenExclusionModal}
                  copySourceHouseId={copySourceHouseId}
                  setCopySourceHouseId={setCopySourceHouseId}
              ...
              {view === 'admin-plot' && isAdmin && (
                <AdminPlotView
                  ...
                  batchPlots={batchPlots}
                  setBatchPlots={setBatchPlots}
                  houseTypes={houseTypes}
                  foremenList={foremenList}
                  isSubmitting={isSubmitting}
          ...
          {isMobileLayout && (
            <nav className="bg-white border-t border-slate-200 p-2 fixed bottom-0 left-[14px] right-[14px] rounded-b-[2rem] z-[100] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] flex justify-around">
              ...
              <button onClick={() => { setView('dashboard'); setSelectedProject(null); }} className={`flex flex-col items-center p-2 rounded-xl w-16 ${activeView === 'dashboard' ? 'text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}>
              ...
              {(isAdmin || isProcurement) && (
                <>
                  <button onClick={() => setView('billing-queue')} className={`flex flex-col items-center p-2 rounded-xl w-16 ${activeView === 'billing-queue' ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
                    <FileSpreadsheet size={20} className={activeView === 'billing-queue' ? 'fill-indigo-100' : ''} />
                    <span className="text-[9px] font-black mt-1">คิวเบิก</span>
                  </button>
                  ...
                  <button onClick={() => setView('procurement-dashboard')} className={`flex flex-col items-center p-2 rounded-xl w-16 ${activeView === 'procurement-dashboard' ? 'text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}>
                    <Users size={20} className={activeView === 'procurement-dashboard' ? 'fill-emerald-100' : ''} />
                    <span className="text-[9px] font-black mt-1">ระบุช่าง</span>
                  </button>
                </>
              )}
      ...
      {/* 🌟 Bulk N/A (Exclusion) Modal 🌟 */}
      {exclusionModal.isOpen && (
        <div className="absolute inset-0 z-[600] bg-black/40 backdrop-blur-xl flex items-center justify-center p-4 fixed transition-all">
          <div className="bg-white rounded-[2rem] shadow-2xl max-w-md w-full p-6 space-y-5 animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
            <div>
              <h3 className="text-xl font-black text-slate-800 italic uppercase">ข้อยกเว้นงาน (N/A)</h3>
              <p className="text-sm text-slate-500 mt-1">
                ติ๊กเลือกแปลงที่ <span className="font-bold text-rose-500">ไม่มี</span> งาน <strong>{exclusionModal.task?.task_name}</strong>
              </p>
            </div>
            <div className="flex-1 overflow-y-auto min-h-0 space-y-2 pr-2">
              {exclusionModal.plots.map((plot: any, idx: number) => (
                <label key={plot.id} className={`flex items-center justify-between p-3 border rounded-xl cursor-pointer transition-colors ${plot.is_excluded ? 'bg-orange-50 border-orange-200' : 'border-slate-200 hover:bg-slate-50'}`}>
                  <div className="flex flex-col">
                    <span className={`font-bold ${plot.is_excluded ? 'text-orange-700' : 'text-slate-700'}`}>แปลง: {plot.plot_name || plot.id}</span>
                    <span className={`text-xs ${plot.is_excluded ? 'text-orange-500' : 'text-slate-400'}`}>{plot.project_name}</span>
                  </div>
                  <input 
                    type="checkbox" 
                    className="w-5 h-5 rounded border-slate-300 text-orange-500 focus:ring-orange-500 cursor-pointer"
                    checked={plot.is_excluded}
                    onChange={(e) => {
                      const newPlots = [...exclusionModal.plots];
                      newPlots[idx].is_excluded = e.target.checked;
                      setExclusionModal({ ...exclusionModal, plots: newPlots });
                    }}
                  />
                </label>
              ))}
            ...
            <div className="flex gap-3 pt-2 shrink-0">
              <button onClick={() => setExclusionModal({ isOpen: false, task: null, plots: [] })} disabled={isSubmitting} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3 rounded-xl transition-colors">
                ยกเลิก
              </button>
              <button onClick={handleSaveExclusion} disabled={isSubmitting} className="flex-1 bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2">
                {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />} บันทึก
              </button>
            </div>
          </div>
        </div>
        ...
        return (
          <div className="fixed inset-0 z-[99999] bg-slate-900 flex flex-col animate-in fade-in duration-300">
              ...
              <div className="flex items-center gap-3 sm:gap-6">
                ...
                <div className="flex items-center gap-1.5 sm:gap-2 bg-slate-800 px-2 sm:px-3 py-1.5 rounded-lg border border-slate-700">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        ...
                        if (!val) return;
                        const idx = presentationPlots.findIndex((p: any) => p.id.toLowerCase() === val.toLowerCase());
                        if (idx !== -1) {
                          setCurrentSlideIndex(idx);
                          (e.target as HTMLInputElement).value = ''; // clear after jumping
                          (e.target as HTMLInputElement).blur(); // remove focus
                        } else {
                          showAlert('ข้อผิดพลาด', 'ไม่พบข้อมูลแปลง: ' + val);
                        }
                      }
                    }}
                  />
                  ...
                  <datalist id="plot-search-list-top">
                    {presentationPlots.map((p: any) => <option key={p.id} value={p.id} />)}
                  </datalist>
                </div>
                <span className="hidden sm:inline-block text-slate-400 font-bold text-sm bg-slate-800 px-3 py-1.5 rounded-lg shrink-0">สไลด์ {currentSlideIndex + 1} / {presentationPlots.length}</span>
                <span className="sm:hidden text-slate-400 font-bold text-xs shrink-0">{currentSlideIndex + 1}/{presentationPlots.length}</span>
            ...
            <div className="flex-1 overflow-hidden flex items-center justify-center p-4 sm:p-8 relative w-full h-full">
              ...
              <button onClick={() => setCurrentSlideIndex(p => Math.min(p + 1, presentationPlots.length - 1))} disabled={currentSlideIndex === presentationPlots.length - 1} className="absolute right-2 sm:right-6 p-3 sm:p-5 bg-white/10 hover:bg-white/20 text-white rounded-full disabled:opacity-0 transition z-50 backdrop-blur-md">
                <ChevronRight size={32} />
              </button>
                              ...
                              {Array.from(new Set(plotUpdates.map((u: any) => u.task_template_id))).map((taskId: any, tIdx) => {
                                ...
                                if (taskProgress === 100 && actualEndRaw !== '-') {
                                    ...
                                    if (taskAction === 'QC อนุมัติ' || taskAction === 'QC อนุมัติผ่าน' || (!tTemplate?.require_qc && taskAction === 'Site Engineer อนุมัติ')) {
                                      ...
                                      actualEndUI = <div className="flex items-center justify-center gap-1 text-[10px] sm:text-[11px] font-bold text-yellow-700 whitespace-nowrap bg-yellow-50 px-2 py-0.5 rounded w-[110px] text-center"><span>{actualEndRaw}</span><span className="text-[8px] bg-yellow-200 text-yellow-800 px-1 rounded-sm">⏳ รอ Site Engineer</span></div>;
                                    } else {
      ...
      {/* 🌟 Global Upload State Popup */}
      {(globalUploadState.isUploading || globalUploadState.isSuccess) && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full flex flex-col items-center justify-center text-center animate-in zoom-in-95 duration-300">
            {globalUploadState.isUploading ? (
              <>
                <Loader2 size={48} className="text-blue-500 animate-spin mb-4" />
                <h3 className="text-lg font-bold text-slate-800 mb-2">กำลังทำงาน...</h3>
                <p className="text-sm text-slate-500">{globalUploadState.message}</p>
              </>
            ) : (
              <>
                <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mb-4 animate-in zoom-in-50 duration-500">
