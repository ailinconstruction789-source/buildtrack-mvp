                                ...
                                return (
                                  <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                                    ...
                                    <td className="p-4 pr-6 sm:pr-8 text-center">
                                      <span className={`font-black px-3.5 py-1.5 rounded-xl text-xs sm:text-sm shadow-sm inline-block whitespace-nowrap border ${defect.status === 'pending'
          ...
          <div className="fixed inset-0 z-[99999] bg-slate-900 flex flex-col animate-in fade-in duration-300">
            ...
            <div className="flex-1 overflow-hidden flex items-center justify-center p-4 sm:p-8 relative w-full h-full">
              ...
              <div className="bg-slate-50 w-full max-w-7xl aspect-[4/3] sm:aspect-[16/9] rounded-2xl sm:rounded-[2rem] shadow-2xl flex flex-col sm:flex-row overflow-hidden border border-slate-700 max-h-full">
                        ...
                        {plotUpdates.length > 0 ? (
                          <div className="space-y-3 max-h-[35vh] overflow-y-auto pr-1 custom-scrollbar">
                            ...
                            {Array.from(new Set(plotUpdates.map((u: any) => u.action))).map((taskAction: any, tIdx) => {
                              ...
                              return (
                                <div key={tIdx} className="bg-white border border-slate-200 rounded-xl shadow-sm hover:border-indigo-200 transition-colors overflow-hidden">
                                  <div className="p-3.5 sm:p-4">
                                    <div className="flex flex-col xl:flex-row gap-4 xl:gap-6">
                                      {/* Left Column: Task Info */}
                                      <div className="xl:w-2/5 flex flex-col gap-2">
                                        <div className="flex justify-between items-start">
                                          <div className="flex items-center gap-2">
                                            <span className="bg-slate-100 text-slate-500 font-black text-xs px-2 py-0.5 rounded">#{tIdx + 1}</span>
                                            <span className="font-black text-slate-800 text-sm sm:text-base leading-tight">{taskAction}</span>
                                          </div>
                                        </div>
                                        <div className="flex flex-col gap-1 mt-1">
                                          <div className="flex items-center gap-1.5 text-xs font-bold text-blue-600 bg-blue-50 w-fit px-2 py-1 rounded-md">
                                            <HardHat size={12} /> {contractorName}
                                          </div>
                                          {matchedAssign?.contractor_phone && (
                                            <div className="flex items-center gap-1.5 text-[10px] sm:text-xs font-medium text-slate-500 px-2 mt-0.5">
                                              📞 {matchedAssign.contractor_phone}
                                            </div>
                                          )}
                                        </div>
                                        <div className="mt-2 flex items-center gap-3">
                                          <div className="flex-1 bg-slate-100 rounded-full h-2 overflow-hidden shadow-inner">
                                            <div className="bg-indigo-500 h-2 rounded-full transition-all duration-500" style={{ width: `${taskProgress}%` }}></div>
                                          </div>
                                          <span className="text-indigo-600 text-sm font-black shrink-0 w-8 text-right">{taskProgress}%</span>
                                        </div>
                                        <div className="mt-1">
                                          <span className={`text-[10px] sm:text-xs font-bold border py-0.5 px-2 rounded-md ${delayColor}`}>
                                            {delayStatusStr}
                                          </span>
                                        </div>
                                      </div>
                                      ...
                                      <div className="xl:w-3/5 border-t xl:border-t-0 xl:border-l border-slate-100 pt-3 xl:pt-0 xl:pl-6 flex items-center">
                                        <div className="w-full">
                                          {/* Desktop Table Header (hidden on mobile) */}
                                          <div className="hidden sm:grid grid-cols-3 gap-2 mb-2 px-2 border-b border-slate-100 pb-2">
                                            <div className="text-xs font-black text-slate-400 uppercase tracking-widest text-center">Start</div>
                                            <div className="text-xs font-black text-rose-500 uppercase tracking-widest text-center">Duration</div>
                                            <div className="text-xs font-black text-slate-400 uppercase tracking-widest text-center">Finish</div>
                                          </div>

                                          {/* Plan Row */}
                                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-2 mb-2 sm:mb-0 p-2 sm:p-0 bg-slate-50 sm:bg-transparent rounded-lg sm:rounded-none border sm:border-0 border-slate-100">
                                            <div className="flex justify-between sm:block text-center border-b sm:border-b-0 border-slate-100 pb-1 sm:pb-0 mb-1 sm:mb-0">
                                              <span className="sm:hidden text-[10px] font-bold text-slate-400 uppercase">Plan Start</span>
                                              <div className="flex sm:justify-center items-center gap-1.5">
                                                <span className="hidden sm:inline text-[9px] font-black text-slate-400 bg-slate-100 px-1 rounded">PLAN:</span>
                                                <span className="text-xs sm:text-sm font-black text-slate-700">{planStart}</span>
                                              </div>
                                            </div>
                                            <div className="flex justify-between sm:block text-center border-b sm:border-b-0 border-slate-100 pb-1 sm:pb-0 mb-1 sm:mb-0">
                                              <span className="sm:hidden text-[10px] font-bold text-slate-400 uppercase">Plan Duration</span>
                                              <span className="text-xs sm:text-sm font-black text-slate-700">{planDurationStr}</span>
                                            </div>
                                            <div className="flex justify-between sm:block text-center">
                                              <span className="sm:hidden text-[10px] font-bold text-slate-400 uppercase">Plan Finish</span>
                                              <span className="text-xs sm:text-sm font-black text-slate-700">{planEnd}</span>
                                            </div>
                                          </div>

                                          {/* Actual Row */}
                                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-2 p-2 sm:p-0 sm:mt-2 bg-indigo-50 sm:bg-transparent rounded-lg sm:rounded-none border sm:border-0 border-indigo-100">
                                            <div className="flex justify-between sm:block text-center border-b sm:border-b-0 border-indigo-100/50 pb-1 sm:pb-0 mb-1 sm:mb-0">
                                              <span className="sm:hidden text-[10px] font-bold text-indigo-400 uppercase">Actual Start</span>
                                              <div className="flex sm:justify-center items-center gap-1.5">
                                                <span className="hidden sm:inline text-[9px] font-black text-blue-500 bg-blue-100 px-1 rounded">ACTUAL:</span>
                                                <span className="text-xs sm:text-sm font-black text-blue-600">{actualStart}</span>
                                              </div>
                                            </div>
                                            <div className="flex justify-between sm:block text-center border-b sm:border-b-0 border-indigo-100/50 pb-1 sm:pb-0 mb-1 sm:mb-0">
                                              <span className="sm:hidden text-[10px] font-bold text-indigo-400 uppercase">Actual Duration</span>
                                              <span className="text-xs sm:text-sm font-black text-blue-600">{actualDurationStr}</span>
                                            </div>
                                            <div className="flex justify-between sm:block text-center">
                                              <span className="sm:hidden text-[10px] font-bold text-indigo-400 uppercase">Actual Finish</span>
                                              <span className="text-xs sm:text-sm font-black text-blue-600">{actualEnd}</span>
                                            </div>
                                          </div>
                ...
                {/* Right Panel: Image Gallery (รวมภาพล่าสุด 6 ภาพ) */}
                <div className="w-full sm:w-7/12 bg-slate-100 p-4 sm:p-6 flex flex-col relative overflow-y-auto custom-scrollbar border-l border-slate-200">
                  <h3 className="font-black text-slate-700 mb-4 flex items-center gap-2 text-xl sm:text-2xl shrink-0"><Camera className="text-slate-500" /> รูปล่าสุดของงานที่กำลังดำเนินการ (Max 6)</h3>
                  {plotImages.length > 0 ? (
                    <div className="flex-1">
                      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 auto-rows-max">
                        {plotImages.slice(0, 6).map((img: any, idx: number) => (
                          <div key={idx} className="relative bg-black rounded-xl overflow-hidden aspect-[4/3] group shadow-sm border border-slate-300">
                            <img
                              src={img.url}
                              onClick={() => setFullImageUrl(img.url)}
                              className="w-full h-full object-cover opacity-90 transition-transform duration-700 group-hover:scale-110 group-hover:opacity-100 cursor-zoom-in"
                              alt={img.action}
                            />
                            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-2.5 pt-8">
                              ...
                              <p className="text-white/70 font-bold text-[9px] sm:text-[10px] text-right">{new Date(img.date).toLocaleString('th-TH', { dateStyle: 'short', timeStyle: 'short' })}</p>
                            </div>
                          </div>
                        ))}
                      </div>
