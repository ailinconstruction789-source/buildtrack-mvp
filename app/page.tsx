...
export default function ConstructionApp() {
  ...
  const getPlotOverallStatus = (plotId: any) => {
    ...
    const plotTasks = taskTemplates.filter(t => t.house_type_id === plotInfo?.house_type_id);
    const countedTasks = plotTasks.filter(t => {
       if (t.is_progress_counted === false) return false;
       const assign = assignments?.find((a: any) => a.plot_id === plotId && a.task_template_id === t.id);
    ...
    if (!countedTasks.length) return { actual: 0, planned: 0, status: 'none', label: 'N/A', colors: 'bg-slate-50 border-slate-300 text-slate-400' };
    ...
    countedTasks.forEach(task => {
      const key = `${plotId}-${task.id}`; 
      const plan = schedules[key]; let plannedProg = 0;
      ...
      const tCost = task.cost ? Number(task.cost) : 0;
      totalPlanned += (plannedProg * tCost);
      totalCost += tCost;
      naivePlanned += plannedProg;
    });

    const actualAvg = plotInfo?.progress ? Math.round(plotInfo.progress) : 0;
    const plannedAvg = totalCost > 0 ? Math.round(totalPlanned / totalCost) : (countedTasks.length > 0 ? Math.round(naivePlanned / countedTasks.length) : 0);
