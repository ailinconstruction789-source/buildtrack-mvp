...
export default function ExecutiveAnalytics({
  loading, projects, plots: rawPlots, taskTemplates, schedules: rawSchedules, latestUpdatesMap: rawLatestUpdatesMap, foremenList, allUpdatesRecord: rawAllUpdatesRecord
}: any) {
  ...
  const activeProjectsList = useMemo(() => {
    return (projects || [])
      ...
      .filter((proj: any) => {
        const projPlots = (rawPlots || []).filter((p: any) => p.project_name === proj.name);
  ...
  }, [rawPlots, activeProjectsList, selectedProjectId]);

  const plots = useMemo(() => {
     return (rawPlots || []).filter((p: any) => activePlots.has(p.id));
  }, [rawPlots, activePlots]);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
        ...
        <div className="w-full sm:w-auto">
          <select 
            ...
            className="w-full sm:w-64 bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:border-blue-500 shadow-sm cursor-pointer"
          >
