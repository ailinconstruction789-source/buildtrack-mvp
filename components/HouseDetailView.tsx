...
interface HouseDetailViewProps {
  ...
  handleForceCompleteAllTasks?: (plotId: any, houseTypeId: any) => void;
  loading?: boolean;
  materialRequests?: any[];
  setMaterialRequests?: (v: any) => void;
  materialReceipts?: any[];
  setMaterialReceipts?: (v: any) => void;
  fetchAllData?: () => void;
...
const HouseDetailView = function HouseDetailView(props: HouseDetailViewProps) {
  const {
    ...
    handleTogglePlotCompleted, getPlotOverallStatus,
  ...
  const [requestMaterialDeliveryMode, setRequestMaterialDeliveryMode] = React.useState('single');
  const [requestMaterialType, setRequestMaterialType] = React.useState('boq');
  const [requestMaterialExtraReason, setRequestMaterialExtraReason] = React.useState('');
  const [requestMaterialExpectedDate, setRequestMaterialExpectedDate] = React.useState('');
  ...
  const handleRequestMaterial = async () => {
    ...
    if (requestMaterialType === 'extra' && !requestMaterialExtraReason.trim()) {
      showAlert ? showAlert('กรุณาระบุเหตุผล', 'โปรดระบุเหตุผลในการสั่งวัสดุนอกเหนือ BOQ') : alert('โปรดระบุเหตุผลในการสั่งวัสดุนอกเหนือ BOQ');
      return;
    }
    ...
    try {
      const { error } = await supabase.from('task_material_requests').insert([{
        ...
        status: 'requested',
        delivery_mode: requestMaterialDeliveryMode,
        material_type: requestMaterialType,
        extra_reason: requestMaterialExtraReason,
        expected_date: requestMaterialExpectedDate || null
      }]);
      if (error) throw error;
      if (fetchAllData) await fetchAllData();
      setRequestMaterialTask(null);
      setRequestMaterialNote('');
      setRequestMaterialDeliveryMode('single');
      setRequestMaterialType('boq');
      setRequestMaterialExtraReason('');
      setRequestMaterialExpectedDate('');
      showToast ? showToast('บันทึกคำขอสั่งวัสดุเรียบร้อยแล้ว', 'success') : alert('บันทึกคำขอสั่งวัสดุเรียบร้อยแล้ว');
    } catch (e: any) {
      showAlert ? showAlert('ข้อผิดพลาด', 'เกิดข้อผิดพลาด: ' + e.message) : alert('เกิดข้อผิดพลาด: ' + e.message);
    } finally {
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col">
            <div className="bg-slate-800 p-4 text-white flex justify-between items-center">
              <div>
                <h3 className="font-bold text-lg">สั่งวัสดุสำหรับงาน</h3>
            ...
            <div className="p-5 flex-1 overflow-y-auto space-y-4">
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">รูปแบบการจัดส่ง</label>
                <div className="flex gap-2">
                  <button onClick={() => setRequestMaterialDeliveryMode('single')} className={`flex-1 py-2 text-sm font-bold rounded-xl border ${requestMaterialDeliveryMode === 'single' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white border-slate-200 text-slate-500'}`}>ส่งรวดเดียว</button>
                  <button onClick={() => setRequestMaterialDeliveryMode('partial')} className={`flex-1 py-2 text-sm font-bold rounded-xl border ${requestMaterialDeliveryMode === 'partial' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white border-slate-200 text-slate-500'}`}>ทยอยส่ง</button>
                </div>
              </div>

              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">วันที่ต้องการใช้ของ (หน้างาน)</label>
                <input 
                  type="date" 
                  value={requestMaterialExpectedDate}
                  onChange={(e) => setRequestMaterialExpectedDate(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:border-blue-500 outline-none"
                />
              </div>

              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">ประเภทวัสดุ</label>
                <div className="flex gap-2">
                  <button onClick={() => setRequestMaterialType('boq')} className={`flex-1 py-2 text-sm font-bold rounded-xl border ${requestMaterialType === 'boq' ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-white border-slate-200 text-slate-500'}`}>ตาม BOQ</button>
                  <button onClick={() => setRequestMaterialType('extra')} className={`flex-1 py-2 text-sm font-bold rounded-xl border ${requestMaterialType === 'extra' ? 'bg-rose-50 border-rose-500 text-rose-700' : 'bg-white border-slate-200 text-slate-500'}`}>นอก BOQ (สั่งเพิ่ม)</button>
                </div>
              </div>

              {requestMaterialType === 'extra' && (
                <div className="animate-in slide-in-from-top-2">
                  <label className="block text-sm font-bold text-rose-700 mb-2">เหตุผลที่สั่งเพิ่ม (บังคับ) *</label>
                  <textarea 
                    value={requestMaterialExtraReason}
                    onChange={(e) => setRequestMaterialExtraReason(e.target.value)}
                    placeholder="เช่น ของเดิมเสียหายตอนขนส่ง, ประเมินผิดพลาด, ลค.ขอเพิ่ม..."
                    className="w-full bg-rose-50 border border-rose-200 rounded-xl p-3 text-sm focus:border-rose-500 outline-none h-20 resize-none"
                  ></textarea>
                </div>
              )}
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:border-blue-500 outline-none h-20 resize-none"
                ></textarea>
              </div>
            </div>
            <div className="p-4 border-t border-slate-100 flex gap-2">
              ...
              >
                {isSubmittingMaterial ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />} ยืนยันสั่งวัสดุ
              </button>
            </div>
          </div>
        </div>
      ...
      {receiptUploadTask && selectedPlot && (() => {
        const currentRequest = materialRequests?.find((r: any) => String(r.plot_id) === String(selectedPlot.id) && String(r.task_template_id) === String(receiptUploadTask.id));
        return (
          <ReceiptUploadModal
            ...
            onSuccess={(newReceipt) => {
              ...
              showToast ? showToast('อัปโหลดรูปรับของเรียบร้อย', 'success') : alert('อัปโหลดรูปรับของเรียบร้อย');
            }}
            plotId={selectedPlot.id}
            taskId={receiptUploadTask.id}
            taskName={receiptUploadTask.task_name}
      ...
      {viewHistoryTask && (() => {
         const currentRequest = materialRequests?.find((r: any) => String(r.plot_id) === String(selectedPlot.id) && String(r.task_template_id) === String(viewHistoryTask.id));
         const history = materialReceipts?.filter((r: any) => currentRequest && r.request_id === currentRequest.id) || [];
             ...
             <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
               <div className="bg-slate-800 p-4 text-white flex justify-between items-center shrink-0">
                 <div>
                   ...
                   <p className="text-xs text-slate-300">{viewHistoryTask.task_name}</p>
                 </div>
                  ...
                  <div className="flex flex-wrap justify-between items-center gap-3">
                    ...
                    <div className="flex-shrink-0 flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                           <h2 className="text-xl font-bold italic tracking-tighter">{selectedPlot.plot_name || selectedPlot.id}</h2>
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 ml-auto items-end sm:items-center">
                      {(isAdmin || isProjectPlanner) && (
                        <div className="flex gap-1.5">
                          ...
                          {isAdmin && handleForceCompleteAllTasks && (
                            <button 
                              onClick={() => handleForceCompleteAllTasks(selectedPlot.id, selectedPlot.house_type_id)}
                              className="px-2.5 py-1.5 rounded text-[10px] sm:text-xs font-bold border border-rose-700 bg-rose-900/50 text-rose-400 hover:bg-rose-600 hover:text-white hover:border-rose-500 transition-all flex items-center gap-1 shadow-sm"
                              title="บังคับให้งานทุกชิ้นเสร็จ 100% (สำหรับใส่ข้อมูลย้อนหลัง)"
                            >
                              ⚡ บังคับเสร็จสิ้น 100%
                            </button>
                          )}
                     ...
                     {activeHouseTab === 'construction' && (() => {
                        const totalPlotCost = taskTemplates.reduce((sum, task) => {
                          const isExcluded = selectedPlot?.house_types?.type_name 
                                          ? task.excluded_house_types?.includes(selectedPlot.house_types.type_name) 
                                          : false;
                          return sum + (isExcluded ? 0 : (Number(task.cost) || 0));
                        }, 0);
                       ...
                       {isMobileLayout && <div className="text-center text-[10px] text-slate-400 font-bold py-2 bg-[#f5f5f7] border-b border-black/5">↔️ ปัดซ้าย-ขวา เพื่อดูตาราง ↔️</div>}
                         ...
                         <thead className="sticky top-0 z-[60] bg-[#f5f5f7] shadow-sm text-[10px] sm:text-xs font-bold uppercase text-[#86868b] tracking-widest">
                           <tr>
                             ...
                             {!isMobileLayout && <th className="sticky left-[280px] bg-[#f5f5f7] z-[65] border-b border-r border-black/5 p-3 sm:p-5 text-center w-[80px] min-w-[80px] max-w-[80px] shadow-[-6px_0_10px_-6px_rgba(0,0,0,0.08)]">Weight</th>}
                         ...
                         <tbody className={isMobileLayout ? 'block p-3 sm:p-0 bg-[#f5f5f7]' : ''}>
                          {props.loading ? (
                            ...
                            const isExcluded = assignment?.is_excluded === true;
                            ...
                            const openTaskProgress = () => {
                              if (isExcluded) {
                                showAlert ? showAlert('ข้อยกเว้นงาน', `งาน "${task.task_name}" ไม่มีในแบบบ้านของแปลงนี้ (N/A)`) : alert(`งาน "${task.task_name}" ไม่มีในแบบบ้านของแปลงนี้ (N/A)`);
                                return;
                              }
                            ...
                            return (
                                <React.Fragment key={task.id}>
                                        ...
                                        <td className="block bg-white rounded-[1.5rem] shadow-[0_8px_30px_-10px_rgba(0,0,0,0.1)] p-5 border border-black/5 relative overflow-hidden">
                                            ...
                                            <div className="flex items-start justify-between mb-4 border-b border-slate-100 pb-3 mt-1">
                                              ...
                                              <div className="flex flex-col items-end gap-1 shrink-0">
                                                {isExcluded ? (
                                                  <div className="px-2.5 py-1 rounded-xl text-xs font-bold bg-slate-200 text-slate-500">N/A</div>
                                                ) : (
                                                  <>
                                            ...
                                            {(currentUserRole === 'Foreman' || currentUserRole === 'Site Engineer' || currentUserRole === 'Project Planner' || currentUserRole === 'Admin' || currentUserRole === 'Owner') && (() => {
                                              const taskRequests = materialRequests?.filter((r: any) => String(r.plot_id) === String(selectedPlot.id) && String(r.task_template_id) === String(task.id)) || [];
                                              const currentRequest = taskRequests.sort((a: any, b: any) => new Date(b.requested_at).getTime() - new Date(a.requested_at).getTime())[0];
                                              if (!currentRequest) {
                                                return (
                                              ...
                                              if (currentRequest.status === 'requested') {
                                                return (
                                                  <div className="flex gap-2 mb-3 w-full">
                                                    <div className="flex-[2] py-3 bg-amber-50 border border-amber-200 text-amber-600 text-[11px] sm:text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 shadow-sm"><Clock size={16}/> รอสโตร์</div>
                                                    <button onClick={(e) => { e.stopPropagation(); setReceiptUploadTask(task); }} className="flex-[1] py-3 bg-blue-50 border border-blue-200 text-blue-600 hover:bg-blue-100 text-[11px] sm:text-xs font-bold rounded-xl active:scale-95 transition-all flex items-center justify-center gap-1.5 shadow-sm">
                                                      <Camera size={16}/> รับเอง
                                                    </button>
                                                  </div>
                                                );
                                                ...
                                                return (
                                                  <div className="flex flex-col gap-1 w-full mb-3">
                                                    {currentRequest.pr_number && (
                                                      <div className="text-[10px] text-indigo-600 font-bold bg-indigo-50 border border-indigo-200 rounded-lg px-2 py-1 flex items-center gap-1"><Package size={12}/> PR: {currentRequest.pr_number}</div>
                                                    )}
                                                    <div className="flex gap-2 w-full">
                                                      <div className="flex-[2] py-3 bg-orange-50 border border-orange-200 text-orange-600 text-[11px] sm:text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 shadow-sm"><Truck size={16}/> รอของเข้า</div>
                                                      <button onClick={(e) => { e.stopPropagation(); setReceiptUploadTask(task); }} className="flex-[1] py-3 bg-blue-50 border border-blue-200 text-blue-600 hover:bg-blue-100 text-[11px] sm:text-xs font-bold rounded-xl active:scale-95 transition-all flex items-center justify-center gap-1.5 shadow-sm">
                                                        <Camera size={16}/> รับของ
                                                      </button>
                                                    </div>
                                                  </div>
                                                );
                                              } else if (currentRequest.status === 'partial') {
                                                return (
                                                  <div className="flex gap-2 mb-3 w-full flex-wrap">
                                                    <button onClick={(e) => { e.stopPropagation(); setViewHistoryTask(task); }} className="flex-[2] py-3 bg-blue-50 border border-blue-200 text-blue-600 hover:bg-blue-100 text-[11px] sm:text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 shadow-sm">
                                                      <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></div> ทยอยเข้า (ดูประวัติ)
                                                    </button>
                                                    <button onClick={(e) => { e.stopPropagation(); setReceiptUploadTask(task); }} className="flex-[1] py-3 bg-blue-50 border border-blue-200 text-blue-600 hover:bg-blue-100 text-[11px] sm:text-xs font-bold rounded-xl active:scale-95 transition-all flex items-center justify-center gap-1.5 shadow-sm">
                                                      <PlusCircle size={16}/> เพิ่มรูป
                                                    </button>
                                                    <button onClick={(e) => { e.stopPropagation(); setRequestMaterialTask(task); }} className="flex-[1] py-3 bg-indigo-50 border border-indigo-200 text-indigo-600 hover:bg-indigo-100 text-[11px] sm:text-xs font-bold rounded-xl active:scale-95 transition-all flex items-center justify-center gap-1.5 shadow-sm">
                                                      <Truck size={16}/> สั่งเพิ่ม
                                                    </button>
                                                  </div>
                                                );
                                                ...
                                                return (
                                                  <div className="flex gap-2 mb-3 w-full flex-wrap">
                                                    <button onClick={(e) => { e.stopPropagation(); setViewHistoryTask(task); }} className="flex-[2] py-3 bg-emerald-50 border border-emerald-200 text-emerald-600 hover:bg-emerald-100 text-[11px] sm:text-xs font-bold rounded-xl active:scale-95 transition-all flex items-center justify-center gap-1.5 shadow-sm">
                                                      <CheckCircle size={16}/> ของครบ (ประวัติ)
                                                    </button>
                                                    <button onClick={(e) => { e.stopPropagation(); setReceiptUploadTask(task); }} className="flex-[1] py-3 bg-blue-50 border border-blue-200 text-blue-600 hover:bg-blue-100 text-[11px] sm:text-xs font-bold rounded-xl active:scale-95 transition-all flex items-center justify-center gap-1.5 shadow-sm">
                                                      <PlusCircle size={16}/> เพิ่มรูป
                                                    </button>
                                            ...
                                            <div className="flex gap-2">
                                              ...
                                              <button onClick={(e) => { e.stopPropagation(); openTaskProgress(); }} className={`flex-[2] py-3 ${isExcluded ? 'bg-slate-200 border-slate-300 text-slate-400 cursor-not-allowed' : tProgress === 100 ? 'bg-emerald-600 hover:bg-emerald-500 border-emerald-700 active:scale-95 text-white active:translate-y-[4px] active:border-b-0' : 'bg-slate-800 hover:bg-slate-700 border-slate-900 active:scale-95 text-white active:translate-y-[4px] active:border-b-0'} text-[11px] sm:text-xs font-bold rounded-xl transition-all flex flex-col items-center justify-center gap-1 shadow-md border-b-4`}>
                                                  <Camera size={16} className={isExcluded ? 'text-slate-300' : tProgress === 100 ? 'text-emerald-100' : 'text-blue-300'} /> {isExcluded ? 'ไม่มีงานนี้ (N/A)' : tProgress === 100 ? 'ดูประวัติ / แจ้งซ่อม' : 'อัปเดตความคืบหน้า'}
                                              </button>
                                  ...
                                  {!isMobileLayout && (
                                  <tr className={`group transition-colors cursor-pointer table-row ${isExcluded ? 'bg-slate-50 opacity-80' : 'bg-white hover:bg-slate-50/80'}`} onClick={(e: any) => { 
                                    ...
                                    <div className="flex flex-col gap-1 mt-1 border-t border-slate-100 pt-1">
                                         ...
                                         {assignment ? (
                                            <div className="flex items-center gap-1.5 min-w-0 flex-1">
                                               ...
                                               <div className="flex flex-col min-w-0 flex-1">
                                                  <span className="text-[10px] sm:text-xs font-bold text-blue-700 truncate">{assignment.contractor_name ? assignment.contractor_name.split(' ')[0] : 'ไม่ระบุ'}</span>
                                       ...
                                       <div className="flex items-center justify-between">
                                          <div className="flex items-center gap-1.5 flex-1 pr-2">
                                             {isExcluded ? (
                                                <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full line-through">N/A</span>
                                             ) : (
                                                <>
                                          ...
                                          <div className="flex items-center gap-1 shrink-0">
                                            ...
                                            {(currentUserRole === 'Foreman' || currentUserRole === 'Site Engineer' || currentUserRole === 'Project Planner' || currentUserRole === 'Admin' || currentUserRole === 'Owner') && (() => {
                                              ...
                                              if (!currentRequest) {
                                                return (
                                                  <button onClick={(e) => { e.stopPropagation(); setRequestMaterialTask(task); }} className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-blue-200 bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center gap-1 transition-colors whitespace-nowrap"><Truck size={10}/> สั่งวัสดุ</button>
                                                );
                                              }
                                              ...
                                              if (currentRequest.status === 'requested') {
                                                return (
                                                  <div className="flex items-center gap-1">
                                                    <span className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-amber-200 bg-amber-50 text-amber-600 flex items-center gap-1 whitespace-nowrap"><Clock size={10}/> รอสโตร์</span>
                                                    <button onClick={(e) => { e.stopPropagation(); setReceiptUploadTask(task); }} className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-blue-200 bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center gap-1 transition-colors whitespace-nowrap"><Camera size={10}/> รับเอง</button>
                                                  </div>
                                                );
                                              } else if (currentRequest.status === 'ordered') {
                                                return (
                                                  <div className="flex flex-col gap-1 justify-center">
                                                    {currentRequest.pr_number && (
                                                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded border border-indigo-200 bg-indigo-50 text-indigo-600 flex items-center gap-1 whitespace-nowrap w-fit"><Package size={10}/> PR: {currentRequest.pr_number}</span>
                                                    )}
                                                    <div className="flex items-center gap-1">
                                                      <span className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-orange-200 bg-orange-50 text-orange-600 flex items-center gap-1 whitespace-nowrap"><Truck size={10}/> รอของเข้า</span>
                                                      <button onClick={(e) => { e.stopPropagation(); setReceiptUploadTask(task); }} className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-blue-200 bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center gap-1 transition-colors whitespace-nowrap"><Camera size={10}/> รับของ</button>
                                                    </div>
                                                  </div>
                                                );
                                              } else if (currentRequest.status === 'partial') {
                                                return (
                                                  <div className="flex items-center gap-1">
                                                    <button onClick={(e) => { e.stopPropagation(); setViewHistoryTask(task); }} className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-blue-200 bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center gap-1 transition-colors whitespace-nowrap"><div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></div> ทยอยเข้า</button>
                                                    <button onClick={(e) => { e.stopPropagation(); setReceiptUploadTask(task); }} className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-blue-200 bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center gap-1 transition-colors whitespace-nowrap"><PlusCircle size={10}/> เพิ่มรูป</button>
                                                    <button onClick={(e) => { e.stopPropagation(); setRequestMaterialTask(task); }} className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-indigo-200 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 flex items-center gap-1 transition-colors whitespace-nowrap"><Truck size={10}/> สั่งเพิ่ม</button>
                                                  </div>
                                                );
                                              } else if (currentRequest.status === 'received') {
                                                return (
                                                  <div className="flex items-center gap-1">
                                                    <button onClick={(e) => { e.stopPropagation(); setViewHistoryTask(task); }} className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-emerald-200 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 flex items-center gap-1 transition-colors whitespace-nowrap"><CheckCircle size={10}/> ของครบ (ประวัติ)</button>
                                                    <button onClick={(e) => { e.stopPropagation(); setReceiptUploadTask(task); }} className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-blue-200 bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center gap-1 transition-colors whitespace-nowrap"><PlusCircle size={10}/> เพิ่มรูป</button>
                                                    <button onClick={(e) => { e.stopPropagation(); setRequestMaterialTask(task); }} className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-indigo-200 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 flex items-center gap-1 transition-colors whitespace-nowrap"><Truck size={10}/> สั่งเพิ่ม</button>
                                                  </div>
                                                );
                                   ...
                                   {(currentUserRole === 'Project Planner' || currentUserRole === 'Site Engineer') && (() => {
                                    ...
                                    return (
                                       <>
                                          ...
                                          {!isMobileLayout && (
  <td className="sticky left-[280px] bg-white z-[40] border-b border-r border-black/5 p-1.5 sm:p-2 align-middle w-[80px] min-w-[80px] max-w-[80px] shadow-[-6px_0_10px_-6px_rgba(0,0,0,0.08)]">
    <div className="flex items-center gap-1 pb-1.5 mb-1.5 border-b border-dashed border-black/5 h-[20px]">
       <span className="text-[8px] font-bold uppercase text-slate-400 w-full text-center">Weight:</span>
    </div>
    <div className="text-[10px] sm:text-xs font-bold text-center text-blue-600 pt-1">
       {(() => {
          const w = Number(task.cost) || 0;
          return totalPlotCost > 0 ? ((w / totalPlotCost) * 100).toFixed(2) + '%' : '-';
       })()}
    </div>
  </td>
)}
<td className={`sticky ${isMobileLayout ? 'left-[220px]' : 'left-[360px]'} bg-pink-50/20 sm:bg-white z-[40] border-b border-r border-black/5 p-1.5 sm:p-2 align-middle w-[115px] sm:w-[140px] min-w-[115px] sm:min-w-[140px] max-w-[115px] sm:max-w-[140px] shadow-[-6px_0_10px_-6px_rgba(0,0,0,0.08)]`}>
                                          ...
                                          <td className={`sticky ${isMobileLayout ? 'left-[335px]' : 'left-[500px]'} bg-pink-50/20 sm:bg-white z-[40] border-b border-r border-black/5 p-1.5 sm:p-2 align-middle w-[70px] sm:w-[100px] min-w-[70px] sm:min-w-[100px] max-w-[70px] sm:max-w-[100px]`}>
                                             <div className="flex items-center gap-1 pb-1.5 mb-1.5 border-b border-dashed border-pink-300">
                                          ...
                                          <td className={`sticky ${isMobileLayout ? 'left-[405px]' : 'left-[600px]'} bg-pink-50/20 sm:bg-white z-[40] border-b border-r border-black/5 p-1.5 sm:p-2 align-middle w-[115px] sm:w-[140px] min-w-[115px] sm:min-w-[140px] max-w-[115px] sm:max-w-[140px] shadow-[6px_0_10px_-6px_rgba(0,0,0,0.1)]`}>
                                             <div className="flex items-center gap-1 pb-1.5 mb-1.5 border-b border-dashed border-pink-300">
...
<td className={`sticky ${isMobileLayout ? 'left-[220px]' : 'left-[360px]'} bg-white z-[40] border-b border-r border-black/5 p-1.5 sm:p-2 align-middle w-[115px] sm:w-[140px] min-w-[115px] sm:min-w-[140px] max-w-[115px] sm:max-w-[140px] shadow-[-6px_0_10px_-6px_rgba(0,0,0,0.08)]`}>
                                          ...
                                          <td className={`sticky ${isMobileLayout ? 'left-[335px]' : 'left-[500px]'} bg-white z-[40] border-b border-r border-black/5 p-1.5 sm:p-2 align-middle w-[70px] sm:w-[100px] min-w-[70px] sm:min-w-[100px] max-w-[70px] sm:max-w-[100px]`}>
                                            <div className="flex items-center gap-1 pb-1.5 mb-1.5 border-b border-dashed border-black/5">
                                         ...
                                         <td className={`sticky ${isMobileLayout ? 'left-[405px]' : 'left-[600px]'} bg-white z-[40] border-b border-r border-black/5 p-1.5 sm:p-2 align-middle w-[115px] sm:w-[140px] min-w-[115px] sm:min-w-[140px] max-w-[115px] sm:max-w-[140px] shadow-[6px_0_10px_-6px_rgba(0,0,0,0.1)]`}>
                                            <div className="flex items-center gap-1 pb-1.5 mb-1.5 border-b border-dashed border-black/5">
