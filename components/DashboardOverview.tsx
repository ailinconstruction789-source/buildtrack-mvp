...
interface DashboardOverviewProps {
  ...
  handleToggleProjectStatus: (projName: string, currentStatus: boolean) => void;
}
...
const DashboardOverview = function DashboardOverview({
  ...
  isMobileLayout,
  projects, plots, taskTemplates, assignments, schedules, plotStatuses, latestUpdatesMap, loggedInUser,
  ...
  handleEditProject, handleToggleProjectStatus
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 w-full mx-auto">
        {(isSiteEngineer || isQC || isAdmin || isOwner || isForeman || isProcurement || isStore || isProjectPlanner || loggedInUser?.role === 'Sales') && (
        ...
        <div className="flex items-center gap-4">
          ...
          {(isAdmin || isOwner) && (
            ...
            >
              {showClosedProjects ? 'ซ่อนโครงการที่ปิดแล้ว' : 'แสดงโครงการที่ปิดแล้ว'}
            </button>
          )}
        </div>
        {isMobileLayout && (
      ...
      <div className={`grid gap-4 sm:gap-6 ${isMobileLayout ? 'grid-cols-1' : 'grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}`}>
        {projects.filter(p => (showClosedProjects ? p.is_closed : !p.is_closed)).map((proj) => {
          let plannedAvg = 0;
          ...
          <div key={proj.name} onClick={() => { 
              ...
              setSelectedProject(proj); setMapGrid(proj.layout_data?.filter((c: any) => c.type !== 'config') || []); setIsEditMapMode(false); setView(loggedInUser?.role === 'Sales' ? 'sales-dashboard' : 'project-detail'); 
            ...
            {(isAdmin || isOwner) && (
              <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 z-20">
                {isAdmin && (
                  <button onClick={(e) => { e.stopPropagation(); handleEditProject(proj); }} className="p-2.5 bg-black/5 backdrop-blur-md text-[#86868b] rounded-full hover:bg-[#1d1d1f] hover:text-white shadow-sm" title="แก้ไขชื่อโครงการ">
                    <Settings size={18} />
                  </button>
                )}
                <button onClick={(e) => { e.stopPropagation(); handleToggleProjectStatus(proj.name, proj.is_closed); }} className={`p-2.5 backdrop-blur-md rounded-full shadow-sm ${proj.is_closed ? 'bg-emerald-50 text-emerald-600 hover:bg-emerald-500 hover:text-white' : 'bg-rose-50 text-rose-600 hover:bg-rose-500 hover:text-white'}`} title={proj.is_closed ? 'เปิดโครงการกลับมา' : 'ปิดโครงการนี้ (Archive)'}>
                  {proj.is_closed ? <CheckCircle size={18} /> : <XCircle size={18} />}
                </button>
              </div>
            )}
